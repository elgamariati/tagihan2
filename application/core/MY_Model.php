...
//        $columnOrder = $request['columns'][$order['column']]['data'];
     ...
     * @return mixed
     */
    function getByPrimary($id, $asArray = TRUE, $relation = FALSE) {
        if ($id == NULL) {
            return NULL;
        }
        $this->db->where($this->pK, $id);
    ...
    public function getOneSelect($where,$select ="*",$asArray = TRUE,$relation = FALSE){
        $this->db->select($select);
        $this->db->where($where);
        if($relation){
            foreach($this->relations() as $alias=>$relasi){
    ...
    public function exist_multi_parameter($value){
        $this->db->where($value);
        return ($this->db->count_all_results($this->table) > 0);
    }
