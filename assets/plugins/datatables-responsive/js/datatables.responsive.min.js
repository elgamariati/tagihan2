...
function ResponsiveDatatablesHelper(t, e) {
...
ResponsiveDatatablesHelper.prototype.init = function(t) {
    ...
    for (var i = 0; i < e.length; i++) this.breakpoints[e[i].name] = e[i];
    for (var o = this.tableElement.fnSettings().aoColumns, i = 0; i < o.length; i++) o[i].bVisible && this.columnIndexes.push(i);
    var s = $("thead th", this.tableElement);
    _.each(s, function(t, e) {
        "expand" === $(t).attr("data-class") && (this.expandColumn = e);
        var n = $(t).attr("data-hide");
        if (void 0 !== n) {
            var i = n.split(/,\s*/);
            _.each(i, function(t) {
    ...
    if (!this.disabled) {
        ...
        if (i && (this.skipNextWindowsWidthChange = !0, this.columnsHiddenIndexes = n, this.columnsShownIndexes = _.difference(this.columnIndexes, this.columnsHiddenIndexes), this.showHideColumns(), e = this.columnsHiddenIndexes.length, this.skipNextWindowsWidthChange = !1), this.columnsHiddenIndexes.length) {
            ...
            var a = this;
            $("tr.detail-show", this.tableElement).each(function(t, e) {
                var n = $(e);
                0 === n.next(".row-detail").length && ResponsiveDatatablesHelper.prototype.showRowDetail(a, n)
            })
        } else this.tableElement.removeClass("has-columns-hidden"), $("tr.row-detail").remove()
    }
}, ResponsiveDatatablesHelper.prototype.showHideColumns = function() {
    for (var t = 0, e = this.columnsShownIndexes.length; e > t; t++) this.tableElement.fnSetColumnVis(this.columnsShownIndexes[t], !0, !1);
    for (var t = 0, e = this.columnsHiddenIndexes.length; e > t; t++) this.tableElement.fnSetColumnVis(this.columnsHiddenIndexes[t], !1, !1);
    var n = this;
    $("tr.row-detail").remove(), this.tableElement.hasClass("has-columns-hidden") && $("tr.detail-show", this.tableElement).each(function(t, e) {
        ResponsiveDatatablesHelper.prototype.showRowDetail(n, $(e))
    })
}, ResponsiveDatatablesHelper.prototype.createExpandIcon = function(t) {
    if (!this.disabled) {
        var e = $("td", t);
        if (void 0 !== this.expandColumn && this.expandColumn < e.length) {
            var n = $(e[this.expandColumn]);
            0 == $("span.responsiveExpander", n).length && (n.prepend(this.expandIconTemplate), n.on("click", "span.responsiveExpander", {
    ...
    if (!this.disabled) {
        ...
        e.hasClass("detail-show") ? ResponsiveDatatablesHelper.prototype.hideRowDetail(t.data.responsiveDatatablesHelperInstance, e) : ResponsiveDatatablesHelper.prototype.showRowDetail(t.data.responsiveDatatablesHelperInstance, e), e.toggleClass("detail-show"), t.stopPropagation()
    }
    ...
    var n = t.tableElement,
        ...
        o = $(t.rowTemplate),
        s = $("ul", o);
    _.each(t.columnsHiddenIndexes, function(o) {
        var a = $(t.rowLiTemplate);
        $(".columnTitle", a).html(i[o].sTitle), a.append(n.fnGetData(e[0], o)), s.append(a)
    });
    var a = t.columnIndexes.length - t.columnsHiddenIndexes.length;
    $("td", o).attr("colspan", a), e.after(o)
